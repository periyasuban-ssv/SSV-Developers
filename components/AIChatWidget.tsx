...
const AIChatWidget: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      role: "model",
      text: "Hello! I'm the SSV Developers AI Assistant. I can help with project estimates, construction advice, or design ideas. Please note that AI features require an API key connection.",
    },
  ]);
    ...
    try {
      ...
      if (aiStudio) {
        ...
        setMessages((prev) => [
          ...
          {
            role: "model",
            text: "API Key connected! How can I assist you with your construction queries now?",
          },
        ]);
      }
    } catch (e) {
    ...
    try {
      ...
      if (error.message === "AUTH_REQUIRED") {
        ...
        setMessages((prev) => [
          ...
          {
            role: "model",
            text: "I'm having some trouble connecting. Please check your internet or try reconnecting your API key.",
            isError: true,
          },
        ]);
      }
      ...
      >
        ...
        <div className="bg-slate-900 text-white p-6 flex justify-between items-center relative overflow-hidden">
          ...
          <div className="flex items-center gap-4 relative z-10">
            <div
              className={`p-2.5 rounded-2xl transition-colors ${!needsKey ? "bg-amber-500" : "bg-slate-700"}`}
            >
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 flex items-center gap-1.5 font-bold uppercase tracking-widest">
                <span
                  className={`w-2 h-2 rounded-full ${!needsKey ? "bg-green-500 animate-pulse" : "bg-amber-500"}`}
                ></span>
          ...
          <button
            ...
            className="text-slate-400 hover:text-white transition-all hover:rotate-90 relative z-10"
          >
            <X className="w-6 h-6" />
          </button>
        </div>
        ...
        <div className="p-5 bg-white border-t border-slate-100">
          ...
          <div className="flex justify-between items-center mt-4 px-1">
            ...
            <button
              onClick={handleConnectKey}
              className="text-[9px] text-amber-600 font-black uppercase tracking-widest hover:underline"
            >
              Re-connect Key
            </button>
          </div>
